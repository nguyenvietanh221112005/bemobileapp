...
class AddressModel {
  ...
  static async updateAddressById(id, data) {
    ...
    const [current] = await db.execute(`SELECT * FROM dia_chi WHERE id = ? AND nguoi_dung_id = ?`, [
      id,
      data.nguoi_dung_id,
    ]);
  ...
  static async createAddress(data) {
    ...
    const [existing] = await db.execute(`SELECT COUNT(*) AS count FROM dia_chi WHERE nguoi_dung_id = ?`, [
      data.nguoi_dung_id,
    ]);
  ...
  static async deleteAddressById(id, nguoi_dung_id) {
    const [result] = await db.execute(`DELETE FROM dia_chi WHERE id = ? AND nguoi_dung_id = ?`, [id, nguoi_dung_id]);
